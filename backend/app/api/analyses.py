...
from app.api.serializers import analysis_detail, analysis_summary, match_out, passage_out, section_out
...
def download_report(
    ...
    return Response(
        content,
        media_type=media,
        headers={"Content-Disposition": f'attachment; filename="{base}_report.{format}"', "Cache-Control": "no-store"},
    )
